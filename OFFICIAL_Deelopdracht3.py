"""
In dit script wordt er gekeken naar de inventarisatie van de reads na het trimmen.
...
def main():
    with open(snakemake.input.trim_fw, 'r') as file, open(snakemake.input.trim_rv, 'r') as file2, open(snakemake.output.trim_data, "w") as f:
        count = -1
        lengte_reads_f = 0
        lengte_reads_r = 0
        minimum_read_f = 1000
        ...
        while True:
            ...
            if len(seq_f)<minimum_read_f and len(seq_f) != 0:
                minimum_read_f = len(seq_f)
            if len(seq_f)>maximum_read_f and len(seq_f) != 0:
            ...
            #f.write("FORWARD GC content: %s \n" % (gc_content_f))
            lengte_reads_r += len(seq_r)
            if len(seq_r)<minimum_read_r and len(seq_r) != 0:
                minimum_read_r = len(seq_r)
            if len(seq_r)>maximum_read_r and len(seq_r) != 0:
...
def usage_information():
    print("""
    Usage Information Deelopdracht 3
    
    i. Functie van het script
    Dit script voert een inventarisatie uit van twee FastQ bestanden,
    Maar nu na het trimmen van de reads. Op deze manier kan je goed de verschillen zien.
    ...
    snakemake --snakefile snakefile_s1101573 three
